...
A protein FASTA file containing gene annotations is generated based on the consensus sequence with \texttt{Prokka} and can be used for detailed gene expression analysis. \todo{need for tool for detection of amino acid changes, better to decode adaptations with amino acids than on nucleotide level. goal: to see what exactly are the differences between the strains} \\
...
\todo{snipit plot: even though large plot, provides insight into the distribution of SNPs -> evenly distributed or recombinant/half genome and BLast match was only covering parts of the examined genome}
